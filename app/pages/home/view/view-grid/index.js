...
class ViewGrid extends React.Component {
	...
	render() {
		...
		let iconDisplay = file => {
			...
			switch (file.type) {
				case 'dir':
					if (this.state.fileIdHovered == file.id && !this.isFileSelected(file.id)) {
						props.icon = 'folder-open';
					} else if (this.props.files.selectionAction == 'drag' && this.isFileSelected(file.id)) {
		...
		return (
				<div className="view-grid">
					...
					{this.props.files.selectionAction}
				</div>
		);
	}
}
